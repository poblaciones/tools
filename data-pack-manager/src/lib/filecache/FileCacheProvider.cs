...
namespace NHibernate.Caches.FileCache
{
	...
	public class FileCacheProvider : ICacheProvider
	{
        ...
        static FileCacheProvider()
		{
            log = LoggerProvider.LoggerFor(typeof(FileCacheProvider));
			caches = new Dictionary<string, ICache>();

			var list = ConfigurationManager.GetSection("filecache") as CacheConfig[];
			if (list != null)
			{
				foreach (CacheConfig cache in list)
				{
                    caches.Add(cache.Region, new FileCache(cache.Region, cache.Properties));
				}
			}
		}
		...
		public ICache BuildCache(string regionName, IDictionary<string, string> properties)
		{
            ...
            return new FileCache(regionName, properties);
		}
