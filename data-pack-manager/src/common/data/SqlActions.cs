...
namespace medea.Data
{
	...
	public class SqlActions
	{
		...
		public void SaveToDisk(CommandType commandType, string commandText, bool doNotFlush, params IDbDataParameter[] commandParameters)
		{
			if (commandParameters.Length > 0)
				throw new Exception("No soportado");

			// Nombre del archivo donde se guardar·n los comandos
			string fileName = "c:\\temp\\comandos_sql.txt";

			// Crear el texto del comando
			StringBuilder commandBuilder = new StringBuilder();
			commandBuilder.AppendLine($"-- Comando: {commandText}");
			commandBuilder.AppendLine($"-- Tipo: {commandType}");

			// Procesar los par·metros
			string processedCommandText = commandText;
			if (commandParameters != null && commandParameters.Length > 0)
			{
				commandBuilder.AppendLine("-- Par·metros:");
				var parameterValues = new Dictionary<string, string>();

				foreach (var param in commandParameters)
				{
					string paramValue = param.Value?.ToString() ?? "NULL";
					paramValue = param.DbType == DbType.String || param.DbType == DbType.DateTime
						? $"'{paramValue.Replace("'", "''")}'"
						: paramValue;

					parameterValues[param.ParameterName] = paramValue;
					commandBuilder.AppendLine($"--     {param.ParameterName} = {paramValue}");

					// Reemplazar el par·metro en el texto del comando
					processedCommandText = processedCommandText.Replace(param.ParameterName, paramValue);
				}
			}

			commandBuilder.AppendLine(processedCommandText);
			commandBuilder.AppendLine(";"); // Asegurar que el comando termine con punto y coma
			commandBuilder.AppendLine(); // LÌnea en blanco para separar comandos

			// Escribir el comando en el archivo
			File.AppendAllText(fileName, commandBuilder.ToString());
			Console.WriteLine($"Comando agregado al archivo: {fileName}");
		}
