...
def get_table_names():
    cnx = pymysql.connect(user=args['MYSQL_USER'], password=args['MYSQL_PASSWORD'], host=args['MYSQL_HOST'], database=args['MYSQL_DB'])
    cursor = cnx.cursor()
...
def main(args):
    ...
    args['quiet'] = (command_args.quiet is not False)
